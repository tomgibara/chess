...
public class BoardTest extends TestCase {
	...
	public void testMoves() {
		...
		List<Move> moves = board.newPositionFor(Colour.WHITE).moves();
	...
	public void testPins() {
		...
		assertMovesAre("b5-b6,b5-b4,b5-b3,b5-b2,b5-b1", board.newPositionFor(BLACK).computeMoves(at("b5").asArea()));
		assertMovesAre("", board.newPositionFor(WHITE).computeMoves(at("f3").asArea()));
		assertMovesAre("h3-g3,h3-f3,h3-h2,h3-h1,h3-g2,h3-f1,h3-g4,h3-f5,h3-e6,h3-d7,h3-c8,h3-h4,h3-h5,h3-h6,h3-h7", board.newPositionFor(BLACK).computeMoves(at("h3").asArea()));
	}
	...
	public void testKingMoves() {
		...
		List<Move> kingMoves = board.newPositionFor(BLACK).computeMoves(kingsArea);
		assertMovesAre("e6-e7", kingMoves);
	}
	
	public void testStopCheck() {
		Board board = Notation.parseFENBoard("8/3k2Q1/8/4qn2/8/3K4/8/8");
		List<Move> moves = board.newPositionFor(BLACK).computeMoves(board.pieces.keySet().asArea());
	...
	public void testEnPassant() {
		...
		List<Move> moves1 = board.newPositionFor(BLACK, ______WG_WC, File.FL_B).computeMoves(area);
		assertTrue(moves1.contains(move("a4-b3")));
		List<Move> moves2 = board.newPositionFor(BLACK, ______WG_WC, null).computeMoves(area);
	...
	public void testCastling() {
		...
		List<Move> moves1 = board.newPositionFor(WHITE, CastlingRights._________WC, null).computeMoves(area);
		assertTrue(moves1.contains(move("e1-c1")));
		List<Move> moves2 = board.newPositionFor(WHITE, CastlingRights.___________, null).computeMoves(area);
		...
		List<Move> moves = board.newPositionFor(WHITE, CastlingRights.______WG_WC, null).computeMoves(area);
